...
export default function ClientMeasurements({setAlert}) {
    ...
    const [loadingAltezza, setLoadingAltezza] = useState(false);
    ...
    async function loadHeight() {
        try {
            setLoadingAltezza(true);
            const altezza = await getUserHeight(id, auth.accessToken);
            setAltezza(altezza);
        } catch (e) {
            setAlert({ message: e.message, type: "danger" });
            console.error(e);
        } finally {
            setLoadingAltezza(false);
        }
    }
                ...
                <div className={`${baseCard} mb-6`}>
                    <div className="flex items-center justify-between border-b border-gray-300 px-4 py-3">
                        ...
                        <div className="flex items-center gap-2 text-sm">
                            <label className="text-gray-600">Altezza(cm)</label>
                            {loadingAltezza ? (
                                <div role="status">
