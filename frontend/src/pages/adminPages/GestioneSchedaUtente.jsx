...
import {deleteExercise, getCliente, getExercises, upsertPlan, loadPlan } from "../../api/editorSchedeApi";
...
export default function CoachWorkoutEditor({setAlert}) {
    ...
    const addToDay = (exercise) => {
        ...
        setPlan((prev) => {
            ...
            next[activeDay] = [
                ...next[activeDay],
                { uid: uuidv4(), id: idVal, name: exercise.name, muscle: exercise.muscle, image: imageUrl, scheme: [{ count: 1, reps: 12 }], rest: 90, notes: "" }
            ];
            return next;
        });
    };
    ...
    const handleSave = async () => {  //Sistemo i dati che voglio salvare lato backend nel db e li salvo
        const payload = {
            userId: id,
            weeks,
            plan: Object.fromEntries(
                Object.entries(plan).map(([dayId, exercises]) => [
                    dayId,
                    exercises.map(ex => ({
                        uid: ex.uid,
                        exerciseId: ex.id || ex._id, // id dell'esercizio dal DB
                        scheme: ex.scheme,
                        rest: ex.rest,
                        notes: ex.notes
                    }))
        ...
        try {
            setSaving(true);
            const res = await upsertPlan(payload, auth.accessToken);
            setAlert({
                message: res.message || "Piano salvato correttamente",
                type: "success",
            })
            //Dal controller potrei già restituire il nuovo piano e aggiornare qui
        } catch (e) {
            console.error("Errore salvataggio piano:", e);
            setAlert({
                type: "danger",
                message: e.response?.data?.message || "Errore nel salvataggio del piano"
            });
        } finally {
            setSaving(false);
        }
    ...
    useEffect(() => {
        // aspetto di avere la libreria caricata per poter fare il join
        ...
        const fetchLatestPlan = async () => {
            try {
                ...
                // ricostruisco il plan nello stesso formato della UI
                const nextPlan = Object.fromEntries(
                    TEMPLATE_DAYS.map(d => {
                        const items = (doc.plan && doc.plan[d.id]) ? doc.plan[d.id] : [];
                        const mapped = items.map(item => {
                            const ex = dict.get(String(item.exerciseId));
                            const imageUrl = ex?.image
                                ? ex.image
                                : (ex?.imagePath ? `${API_BASE}/uploads/exercises/${ex.muscle}/${ex.imagePath}` : "");

                            return {
                                uid: item.uid,
                                id: String(item.exerciseId),
                                name: ex?.name ?? "Esercizio",
                                muscle: ex?.muscle ?? "vario",
                                image: imageUrl,
                                scheme: item.scheme || [],
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-xl p-4 border shadow-sm flex flex-col max-h-[78vh]">
                        ...
                        <div className="sticky bottom-0 bg-white pt-3 mt-3 border-t flex items-center justify-end gap-3">
                            ...
                            <button onClick={handleSave} disabled={saving} className="px-4 py-2 rounded-lg bg-blue-600 text-white">
                                { saving ? (
                                    <div role="status">
                                        <svg aria-hidden="true" className="w-8 h-8 text-gray-200 animate-spin fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
            ...
            {(scheme || []).map((row, idx) => (
                <div key={idx} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] items-center gap-2">
                    ...
                    <button
                        ...
                        className="justify-self-center w-8 h-8 p-0 flex items-center justify-center rounded-md border text-xs hover:bg-gray-50"
